       ...
       //tar standards or implementations
        ...
        switch ($type) {
            ...
            case '5':
                ...
                $array['name'] = MetaBlockAnalyser::getName($meta);
                break;
            //if the $type equals '''L''' or '''K''' it is a specal case in the gnu ustar
            //standard which means that the chunk contains the file name for the next entry
            case 'L':
                case 'K':
                //as the content of the chunk is the name for the next entry we
                //will save for later use
                $name = $this->getContentWithSize($this->chunks[$this->pointer]);
                //increase the current position in the array
                $this->pointer++;
                //get the next entry of the archive
                $array2 = $this->getNextFileOrDirectory();
                //set the name which was in the last chunk to a new array
                $array2['name'] = $name;
                //store the new array in the array which will be returned
                $array = $array2;
                break;
            default:
